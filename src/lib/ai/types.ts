...
/** Результат разбора диктовки: объект/название + заказчик + позиции (PLAN 3.4). */
export interface Extraction {
  /** Объект/название сметы («смета Павлово 2», «объект …»); иначе null. */
  object: string | null;
  /** Заказчик («смета для …», «заказчик …»); иначе null. */
  client: string | null;
  items: ExtractedItem[];
}
