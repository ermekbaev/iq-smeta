  ...
  const rows = d.groups
    .map((g) => {
      const lines = g.lines
        ...
        .join("");
      return `
      <tr class="group"><td colspan="6">${esc(g.label)}</td></tr>
      ${lines}
      <tr class="subtotal"><td colspan="5" class="r">Итого по разделу</td><td class="r">${money(g.subtotal)}</td></tr>`;
    })
    .join("");
...
<html lang="ru"><head><meta charset="utf-8"><style>
  * { box-sizing: border-box; }
  body { font-family: 'DejaVu Sans', Arial, sans-serif; color: #111; font-size: 12px; margin: 32px; }
  ...
  <table>
    <thead><tr>
      <th class="num">№</th><th>Наименование</th><th>Ед.</th>
      <th class="r">Кол-во</th><th class="r">Цена</th><th class="r">Сумма</th>
    </tr></thead>
    <tbody>${rows}</tbody>
  </table>

  <div class="total">ИТОГО: ${money(d.total)} ₽</div>
</body></html>`;
}
