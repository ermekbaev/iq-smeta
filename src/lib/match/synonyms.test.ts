...
describe("applySynonyms", () => {
  ...
  it("сопоставление по границам слов: «бак» не цепляется внутри «табак»", () => {
    expect(applySynonyms(groups, "табак развесной")).toBe("табак развесной");
  });

  it("многословный терм работает как фраза", () => {
    const g = [["гибкая подводка", "шланг"]];
    expect(applySynonyms(g, "нужна гибкая подводка")).toContain("шланг");
  });
});
